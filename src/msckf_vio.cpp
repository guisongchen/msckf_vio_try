...
bool MsckfVio::loadParameters() {
  ...
  // Use variance instead of standard deviation. --squared value
...
#if SFM  
  ric[0] = T_cam0_imu.linear();
  tic[0] = T_cam0_imu.translation();
  ric[1] = T_cam1_imu.linear();
  tic[1] = T_cam1_imu.translation();
  
  cout << "pose cam0 -> imu" << endl;
  cout << ric[0] << endl;
  cout << tic[0].transpose() << endl;
  cout << "pose cam1 -> imu" << endl;
  cout << ric[1] << endl;
  cout << tic[1].transpose() << endl;
#endif
  ...
  // this should be Identity normally, since imu frame is consider as body frame
  IMUState::T_imu_body =
...
bool MsckfVio::initialize() {
  ...
  // diagonal matrix, directly multiply
  ...
  initSFM_ = new initial_sfm::InitSFM(ric, tic);
...
void MsckfVio::imuCallback(const sensor_msgs::ImuConstPtr& msg) {
  ...
  if (!is_gravity_set)
  {
    initial_sfm::ImuPtr  imu_data(new initial_sfm::IMU_MSG());

    imu_data->header_ = msg->header.stamp.toSec();
    tf::vectorMsgToEigen(msg->angular_velocity, imu_data->angularVel_);
    tf::vectorMsgToEigen(msg->linear_acceleration, imu_data->linearAcc_);
    initSFM_->pubImuData(imu_data);
    
    if (initSFM_->initResult())
    {
...
void MsckfVio::initializeGravityAndBias() {
...
#if SFM
  //---- sfm code---------
    state_server.imu_state.gyro_bias = initSFM_->gyrscopeBias();
    Vector3d gravity_imu = initSFM_->gravity();
    
    cout << "g_imu norm: " << gravity_imu.norm()
         << "\ng_imu: " << gravity_imu.transpose() << endl;
    
    Matrix3d R_world_imu;
    double g_imu_norm = gravity_imu.norm();
    Vector3d g_world{0, 0, g_imu_norm};
    R_world_imu = Eigen::Quaterniond::FromTwoVectors(g_world, gravity_imu).toRotationMatrix();
    state_server.imu_state.orientation = rotationToQuaternion(R_world_imu);
    
    cout << "world to imu rotation: " << state_server.imu_state.orientation.transpose() << endl;
  ...
  cout << "gyro_bias: " << state_server.imu_state.gyro_bias.transpose() << endl;
  ...
  cout << "g_imu: " << gravity_imu.transpose() << endl;
...
void MsckfVio::featureCallback(const CameraMeasurementConstPtr& msg) 
{
  ...
  //-------------sfm------------------------
    if (!is_gravity_set)
    {
        ...
        initial_sfm::ImgPtr image(new initial_sfm::IMG_MSG());
        image->header_ = msg->header.stamp.toSec();
        
        for (const auto& feature : msg->features) 
        {
            Vector4d pixel(feature.u0, feature.v0, feature.u1, feature.v1);
            image->features_[static_cast<int>(feature.id)] = pixel;
        }
        
        initSFM_->pubImageData(image);
...
void MsckfVio::batchImuProcessing(const double& time_bound) {
  ...
  // imu data interval:  state_server.header.stamp =< msgs.stamp <= time_bound 
  for (const auto& imu_msg : imu_msg_buffer) {
    ...
    // update X_imu, covariance matrix P
...
void MsckfVio::processModel(const double& time,
  ...
  // make sure P is symmetric matrix
  ...
  // current imu is processed, set this as start time for next process
...
void MsckfVio::predictNewState(const double& dt,
  ...
  // R(t+dt), transpose: from imu to world frame 
  ...
  // k1 = f(tn, yn) -- slope of yn, aka dyn_dt
...
void MsckfVio::stateAugmentation(const double& time) {
  ...
  // read from param, rotation from imu to camera, extrinsic
  const Matrix3d& R_i_c = state_server.imu_state.R_imu_cam0;
  
  // translation from camera to imu, extrinsic
  ...
  // update StateVector
  // cam_states: map<imu_state.id, CAMState>
...
void MsckfVio::measurementUpdate(
  ...
  // Compute the error of the state. delta_X = K * r
  ...
  // from d_theta to dq, can't use simple plus
...
void MsckfVio::removeLostFeatures() {
  ...
  for (auto iter = map_server.begin(); iter != map_server.end(); ++iter) {
    ...
    if (!feature.is_initialized) {
      ...
      // translation from first obs-frame to last obs-frame is small(under threshold), invalid  
      if (!feature.checkMotion(state_server.cam_states)) {
        invalid_feature_ids.push_back(feature.id);
        continue;
      } 
      else // enough translation/paradex to triangulate points 
      {
  ...
  // Process the features which was tracked.
  for (const auto& feature_id : processed_feature_ids) {
    ...
    // get jacobian matrix(multiply left-nullspace) for each feature and related obs-cameras
...
void MsckfVio::pruneCamStateBuffer() {
  ...
  // remove old or remove new, do it twice
  ...
  for (auto& item : map_server) 
  {
    ...
    // make sure current feature is obsered by both removed cameras
...
void MsckfVio::publish(const ros::Time& time) {
  ...
  vio_path.poses.push_back(pose_stamped);
  pub_vio_path.publish(vio_path);

  CAMState& cam_state = state_server.cam_states[imu_state.id];
  Vector3d& trans = cam_state.position;
  Vector4d& quat = cam_state.orientation;

  ofstream pose_output_file("/home/ccc/msckf_pose.txt", ios::app);
  pose_output_file.setf(ios::fixed, ios::floatfield);
  pose_output_file.precision(6);
  pose_output_file << time.toSec() << " ";
  pose_output_file.precision(5);
  pose_output_file << trans.x() << " "
                   << trans.y() << " "
                   << trans.z() << " "
                   << quat.w() << " "
                   << quat.x() << " "
