...
// this part is the primary part of msckf, which propose a model: 
// a static feature from multiple camera poses results in constraints involving all those poses
// 
bool Feature::initializePosition(const CamStateServer& cam_states) {
  ...
  for (auto& m : observations) {
    ...
    // Add the measurement.cam0 and cam1 pixel coordinate
  ...
  // Compute the initial cost for all cameras
  double total_cost = 0.0;
  for (int i = 0; i < cam_poses.size(); ++i) {
    double this_cost = 0.0;
    
    // cost = residual = estimate - measurement
  ...
  do {
    ...
    for (int i = 0; i < cam_poses.size(); ++i) {
      ...
      // J = dr/d(alpha,beta,rou) = dr/dh * dh/d(alpha,beta,rou)
